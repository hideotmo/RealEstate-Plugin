...
function wpestate_search_results_custom($tip=''){
	...
	// Custom search box query for Buy
	$args = '';
	$searchbox = $_REQUEST['searchbox'];
    if($searchbox=='buy') // Buy serach section Query
	{   
	    $requestinputvalue ='';
		$requestinputvalue = $_GET['adv_location'];
		$texo = slugify($requestinputvalue);
		$price_low  = floatval( $_GET['price_low'] );
		$price_max  = floatval( $_GET['price_max'] );
	...
	elseif($searchbox=='sell') // Sell serach section Query
	{
	    $requestinputvalue ='';
		$email_location    = '';
		$requestinputvalue = $_GET['adv_location'];	
		$texo              = slugify($requestinputvalue);
		$email_location    = $_GET['email_location'];	
		$args = array(
			'cache_results'             =>  false,
			'update_post_meta_cache'    =>  false,
			'update_post_term_cache'    =>  false,
	...
	elseif($searchbox=='newsearch')    //advanced search from property detail page
	{
	    $requestinputvalue   = '';
		$email_location      = '';

		$advanced_city       = $_GET['advancedcity'];       // County / State texonomy
		$advanced_contystate = $_GET['district']; // Subdevision - post meta
		//
		$propertytypes       = $_GET['propertytypes'];      // Property Type  texonomy
		$subtypes            = $_GET['property_sub_types']; // Categories texonomy
		$property_style      = $_GET['property_style'];     //Action texonomy
		
		$price_low           = floatval( $_GET['price_low'] ); // Price Ranges - post meta
		$price_max           = floatval( $_GET['price_max'] );	
		
		$pro_size_min        = floatval($_GET['min_size_property']); // property Sizes - post meta
		$pro_size_max        = floatval($_GET['min_size_property']);
		
		$bedrooms            = $_GET['bedrooms']; // Bedrooms - post meta
		$bathrooms           = $_GET['bathrooms'];	// Bathrooms - post meta
		
		$min_property_year   = floatval($_GET['min_property_year']); // - post meta
		$max_property_year   = floatval($_GET['max_property_year']);
		
		// Address - post meta
		$address             = $_GET['address']; 
		// Property_mls - post meta
		$property_mls        = $_GET['property_mls']; 
		
		// Custrom meta query for sidebar search
		
		if($price_low!='' && $price_max!=''):
			$comparearray['key']        = 'property_price';
			$comparearray['value']      = array($price_low, $price_max);
			$comparearray['type']       = 'numeric';
			$comparearray['compare']    = 'BETWEEN';
			$metaquery[]                = $comparearray;										
		endif;
		
		if($pro_size_min!='' && $pro_size_max!=''):
			$comparearray['key']        = 'property_size';
			$comparearray['value']      = array($pro_size_min, $pro_size_max);
			$comparearray['type']       = 'numeric';
			$comparearray['compare']    = 'BETWEEN';
			$metaquery[]                = $comparearray;								
		endif;
		
		if($min_property_year!='' && $max_property_year!=''):
			$comparearray['key']        = 'property-year';
			$comparearray['value']      = array($min_property_year, $max_property_year);
			$comparearray['type']       = 'numeric';
			$comparearray['compare']    = 'BETWEEN';
			$metaquery[]                = $comparearray;			 					   
		endif;
		...
		if($property_mls!=''):
			$comparearray['key']        = 'property_mls';
			$comparearray['value']      = $property_mls;
			$comparearray['type']       = 'CHAR';
			$comparearray['compare']    = 'LIKE';
			$metaquery[]                = $comparearray;									
        endif;

        if($address!=''):
			$comparearray['key']        = 'property_address';
			$comparearray['value']      = $address;
			$comparearray['compare']    = 'LIKE';
			$metaquery[]                = $comparearray;							  
	    endif;
		...
		$args = array(
			...
			'tax_query'       => array(
				   ...
				   array(
						'taxonomy' => 'property_city',
						'field'    => 'slug',
						'terms'    => array($advanced_city),
				   ),
				   array(
						'taxonomy' => 'property_county_state',
						'field'    => 'slug',
						'terms'    => array($advanced_contystate),
				   ),	
				   array(
						'taxonomy' => 'propertytypes',
						'field'    => 'slug',
						'terms'    => array($propertytypes),
				   ),	
				   array(
						'taxonomy' => 'property_category',
						'field'    => 'slug',
						'terms'    => array($subtypes),
				   ),	
				   array(
						'taxonomy' => 'property_action_category',
						'field'    => 'slug',
						'terms'    => array($property_style),
				   ),				   				   			   			   
